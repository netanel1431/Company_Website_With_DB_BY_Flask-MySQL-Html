...
class Change_Password_Current_User():
    ...
    def Check_Password_History_Using(self):
        ...
        print(all_user_current_password)
        for index,_password in enumerate(all_user_current_password):
            if index <=self._check_history:
                if _password==None:
                    continue
                if is_correct_password(stored_password=_password,provided_password=self._input_password):
                    print("class:Change_Password_Current_User--Function:Check_Password_History_Using--The input password len is :{0} allready one of the last {1} password ".format((self._input_password),(self._check_history)))
                    return False
            else:continue
